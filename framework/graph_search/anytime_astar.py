    ...
    def solve_problem(self, problem: GraphProblem) -> SearchResult:
        with Timer(print_title=False) as timer:
            ...
            while (high_heuristic_weight - low_heuristic_weight) > 0.01:
                ...
                temp_res = AStar(heuristic_weight=mid,
                                 max_nr_states_to_expand=self.max_nr_states_to_expand_per_iteration,
                                 heuristic_function_type=self.heuristic_function_type).solve_problem(problem)
                total_nr_expanded_states += temp_res.nr_expanded_states
                max_nr_stored_states = max(max_nr_stored_states, temp_res.max_nr_stored_states)
                if not temp_res.is_solution_found:
                    low_heuristic_weight = mid
                    continue
                if temp_res.solution_cost.get_g_cost() < best_solution.solution_cost.get_g_cost():
                    best_solution = temp_res
